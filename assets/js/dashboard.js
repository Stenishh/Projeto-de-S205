...
async function carregarAulasHoje() {
    const container = document.getElementById('aulasHojeContainer');
    if (!container) return;
    
    mostrarLoading(container);
    
    const horarios = await carregarJSON('assets/data/horarios.json');
    const notas = await carregarJSON('assets/data/notas.json');
    
    if (!horarios) {
        container.innerHTML = '<p style="text-align: center; color: var(--cor-texto-secundario); padding: 20px;">Erro ao carregar horários.</p>';
        return;
    }
    
    const diaHoje = getDiaSemana();
    const aulasDoDia = horarios.find(h => h.dia_cod === diaHoje);
    
    if (!aulasDoDia || !aulasDoDia.aulas || aulasDoDia.aulas.length === 0) {
        container.innerHTML = '<p style="text-align: center; color: var(--cor-texto-secundario); padding: 20px;">😊 Não há aulas hoje! Aproveite seu dia livre.</p>';
        return;
    }
    
    container.innerHTML = '';
    
    aulasDoDia.aulas.forEach(aula => {
        const notaMateria = notas?.find(n => n.materia_id === aula.materia_id);
        const temProvaHoje = verificarProvaHoje(notaMateria);
        
        const aulaElement = document.createElement('div');
        aulaElement.className = 'aula-card';
        
        aulaElement.innerHTML = `
            <div class="aula-header-info">
                <div class="aula-nome">${aula.materia}</div>
            ...
            ${temProvaHoje ? '<div class="labels-aula"><div class="label-prova">⚠️ PROVA HOJE</div></div>' : ''}
            ${notaMateria && notaMateria.cr ? `
                <div class="labels-aula">
                    <span class="badge" style="background-color: ${getCorNota(notaMateria.cr)};">
                        CR: ${notaMateria.cr}
                    </span>
                </div>
            ` : ''}
...
function verificarProvaHoje(notaMateria) {
    if (!notaMateria || !notaMateria.avaliacoes) return false;
    
    const hoje = new Date();
    const hojeStr = `${String(hoje.getDate()).padStart(2, '0')}/${String(hoje.getMonth() + 1).padStart(2, '0')}/${hoje.getFullYear()}`;
    
    return notaMateria.avaliacoes.some(av => av.data === hojeStr);
}
